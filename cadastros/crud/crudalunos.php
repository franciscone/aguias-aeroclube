...
switch ($acao) {
    ...
    case 'inserir':
        ...
        $sql = "INSERT INTO tb_socio (nome_socio, email_socio, end_socio, idade_socio) 
        VALUES ('$nome', '$email', '$endereco', '$idade' )";

        $sql2 = "INSERT INTO tb_telefone (id_socio, ddd_telefone, nm_telefone) 
        VALUES (LAST_INSERT_ID(), '$ddd', '$telefone')";

        $sql3 = "INSERT INTO tb_alunos (id_socio)
        VALUES (LAST_INSERT_ID())";

        if ($_POST['nome'] == ''|| $_POST['email'] == '' || $_POST['endereco'] == '' || $_POST['idade'] == '' || $_POST['ddd'] == '' || $_POST['telefone'] == ''){

            echo "<script>alert('Insira todos os dados para concluir o cadastro')</script> ";

            echo "<script> location.href= '../cadastraralunos.php'; </script>";
    ...
    case 'montar':
        ...
        $id = $_GET['id'];
        $sql = "SELECT * FROM tb_alunos as a
        INNER JOIN tb_socio as s on a.id_socio = s.id_socio
        ...
        while ($registro = mysqli_fetch_array($resultado)) {
            ...
            echo "</div>";
            echo "<form method='post' name='dados' action='crudalunos.php?acao=atualizar' onSubmit='return enviardados();'>";
    ...
    case 'atualizar':
        ...
        $sql = "UPDATE tb_socio s 
        INNER JOIN tb_telefone t ON s.id_socio = t.id_socio 
        SET s.nome_socio = '" . $nome . "', s.email_socio = '" . $email . "', s.end_socio = '" . $endereco . "', s.idade_socio = '" . $idade . "', t.ddd_telefone = '" . $ddd . "', t.nm_telefone = '" . $telefone . "' 
        WHERE s.id_socio = '" . $id . "'";
        ...
        if (!mysqli_query($conn, $sql)) {
            ...
            echo "<script> location.href= 'crudalunos.php?acao=selecionar'; </script>";
        }
        mysqli_close($conn);
    ...
    case 'selecionar':
        ...
        INNER JOIN tb_telefone as t on s.id_socio = t.id_socio
        INNER JOIN tb_alunos as a on s.id_socio = a.id_socio";
        ...
        while ($registro = mysqli_fetch_array($resultado)) {
            ...
            echo "<td>
                <a href='crudalunos.php?acao=montar&id=$id'><img src ='../../imgs/update_crud.png' alt='Atualizar' title='Atualizar registro'></a>

                <a href='../cadastraralunos.php'><img src = '../../imgs/insert_crud.png' alt='Inserir' title='Inserir registro'></a>";
